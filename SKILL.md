...
1. **协议优先 + 日志驱动**：最终交付必须是纯协议脚本（Node.js `final.js` 或 Python `final.py`）。所有加密参数的还原必须以 RuyiTrace NDJSON 日志为优先证据源——先采集日志，再基于日志证据逆向，禁止猜测。工具失败时按"方案梯度"逐级尝试：纯 crypto 还原 → 最小环境复现 → vm 沙箱执行 JS → TLS 指纹模拟。浏览器自动化不在梯度内（见红线 3）。
...
> 取证工具限制见红线 3。下表工具仅用于 Phase 1-3（抓包/trace 采集）。

| 取证工具 | 用途 | AI 自主度 | 浏览器 |
|---|---|---|---|
| ruyipage + RuyiTrace（统一模式） | ruyipage 抓网络包/JS/Cookie + RuyiTrace 采集 NDJSON 环境日志 | 全自主（capture 脚本自动抓） | ruyipage Firefox + RuyiTrace Firefox（仅取证） |
| 用户手动取证 | 用户自行提供 cURL/HAR/JS/RuyiTrace 日志 | 用户驱动 | 按用户选择（仅取证） |

**用户未选择前不启动任何浏览器工具**。默认走 ruyipage + RuyiTrace 统一模式；用户明确要求手动取证时，按用户提供材料分析。
...
**0.1 任务理解（双模式输入）**：
- **完整模式**：用户提供标准请求/响应包（cURL/HAR/原始报文）→ 直接从包中提取信息，跳过抓包
- **极简模式**：用户只提供 URL + 目标参数名（可选）→ Phase 0.5 ruyipage 自动抓包获取信息
...
- 极简模式：启动 ruyipage 轻量抓包（无 hook），navigate(URL) → 提取参数值特征/响应码/JS URL → 下载 JS → 综合判断反爬类型
...
**1.1 抓包分析**：
- ruyipage `page.capture.start()` → 触发请求 → 收集网络包
...
**1.4 入口定位**：详见 `references/workflow/phase-flow.md` Phase 1.4（ruyipage 网络包 → JS 文件定位 → RuyiTrace NDJSON stack 定位签名函数）
...
**2.2 关键词搜索**：
- 在 RuyiTrace 抓的 JS 文件中 Grep 参数名/encrypt/sign/md5/aes
...
**5.3-5.5** 详见 `references/workflow/phase-flow.md`（阶段报告/经验沉淀/清理）
...
- **极简模式**：用户只提供 URL+参数名 → ruyipage 轻量抓包获取上述信息
...
极简模式: 用户提供 URL + 参数名(可选)
  ↓
Phase 0.5 自动识别（轻量 ruyipage，无 hook）:
  1. ruyipage launch（定制 Firefox，有头模式）
  2. page.capture.start(targets=目标接口关键词)
  3. page.get(URL) → 等待页面加载完成
  4. 从抓包结果提取: 参数值特征 + 响应码 + JS 文件 URL
  5. 下载目标 JS 文件
  6. 综合判断反爬类型:
     - 响应码 412 循环 → 签名型 → 补环境
     - JS 文件含 webmssdk/byted_acrawler → 行为型 → 补环境
     - JS 文件 200KB+ + while-switch → JSVMP → 补环境
     - JS 文件含 WASM 加载 → WASM 加密 → WASM 加载
     - JS 文件 <50KB + 无 while-switch + 标准 md5/aes 特征 → 纯算还原
     - JS 文件含 _0x 前缀/obfuscator.io → 纯混淆 → AST反混淆后按算法可提取性判断
  7. 回填 CHECK-3 意图声明
  8. 进入 Phase 1-5
```

**Phase 0.5 抓包结果复用**：极简模式下 Phase 0.5 的 ruyipage 抓包结果（请求列表/Cookie/JS 文件）**直接复用到 Phase 1**，不重复抓包。Phase 1 基于已有抓包结果深入分析。

**注**：Phase 0.5 的 ruyipage 仅用于识别 + 取证，不用于最终交付。用户也可跳过自动识别，直接指定反爬类型。
...
| Phase 0-5 详细流程 | `references/workflow/phase-flow.md` | 分级子流程 |
