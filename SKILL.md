...
description: >
  ...
  通过 L1/L2/L3 三级梯度分流，L1 纯算还原（camoufox MCP 跟踪为可选增强，纯静态分析即可独立完成）,
  L2 MCP 标准分析+vm沙箱，L3 camoufox-reverse 定制版 trace 或 ruyipage+RuyiTrace 复杂场景深度追踪。
...
[CHECK-1] 环境自检 + 工具检测（分阶段，先快后全）
  ...
  第二步：全量检测（仅在用户确认走 L2/L3 或需安装浏览器工具时执行）
    ...
    若 L3 trace 模式，额外加 --require-camoufox-trace 检测 camoufox-reverse 定制版内核
    注: 第一步已确认 camoufox 缺失且用户选择安装时，安装后再执行全量检测

  核心工具判定:
    camoufox = ______ (installed / missing)   ← 仅指默认反检测浏览器
    camoufox-mcp = ______ (installed / missing)   ← 可选，L1 纯算不依赖
    camoufox-trace-kernel = ______ (available / missing)   ← 仅 L3 trace 需要
  通过: Node.js ≥ v18 + camoufox installed → L1 纯算/静态分析即可独立推进
        Node.js ≥ v18 + camoufox+mcp installed → 可走 L1/L2/L3 动态取证增强
...
3. 用户确认 → 运行安装命令：
   ...
   - camoufox（默认反检测浏览器，用于抓包/采样，非 trace 内核）:
       pip install -U "camoufox[geoip]" && python -m camoufox fetch
   - camoufox-reverse-mcp（可选，仅 L1/L2/L3 动态取证增强；不在 pip 官方源，需先克隆再 -e 安装）:
       git clone https://github.com/WhiteNightShadow/camoufox-reverse-mcp.git <install-dir>
       cd <install-dir> && python -m pip install -e .
   - camoufox-reverse 定制版 trace 内核（仅 L3 trace 模式需要，支持 C++ 层属性访问追踪；
     默认 `python -m camoufox fetch` 下载的是普通反检测浏览器，不含 trace 能力，二者不是同一个东西）:
       来源随 camoufox-reverse-mcp 项目提供或单独构建；安装后设置
       CAMOUFOX_REVERSE_BROWSER_PATH=<定制版 firefox 路径> 或 --camoufox-trace-dir <dir>
4. node scripts/precheck_runtime.js（六项纯计算预检）
```

### ⚠️ camoufox 默认版 ≠ camoufox-reverse 定制版（trace 内核）

- `python -m camoufox fetch` 下载的是 **默认反检测浏览器**（daijro/camoufox 上游构建），只用于抓包、采样、轻量取证。
- L3 trace 模式（`launch_browser(enable_trace=True)` / `trace_property_access`）需要 **camoufox-reverse 定制版内核**（支持 C++ 层 trace），它**不是**默认 fetch 的产物。
- 选型与检测：L3 trace 决策（Phase 0.2 / 0.5 判定为 JSVMP / 强风控 / 指纹深度绑定时）才需要 trace 内核；普通 camoufox 安装与 trace 内核安装是两条独立链路，不要混为一谈。
- 检测/提议门禁：选择 L3 trace 后运行 `node scripts/check_external_tools.js --require-camoufox-trace --markdown`；若未检测到定制版内核，必须让用户提供路径 / 确认来源，或明确降级为 ruyiPage + RuyiTrace，**不得用默认 camoufox 静默替代**（否则 trace 功能不可用）。

### ⚠️ camoufox-reverse-mcp 不会自动安装

- MCP 不在 pip 官方源，必须 `git clone` 上游仓库后 `pip install -e .`，且仓库路径需用户提供或确认。
- L1 纯算场景**不依赖** MCP：纯静态分析（curl 下载 JS + Grep 定位）或纯 Node 复现即可独立完成；MCP 只是动态取证的增强项。
- 若用户选择"Camoufox + camoufox-reverse-mcp"但 MCP 未安装，按 `camoufox-tooling.md` 安装门禁处理，不得静默降级为仅 Camoufox（除非用户明确回复"降级为仅 Camoufox"）。

### GitHub 网络不通 → 镜像站代理（必读）

camoufox 浏览器本体、`camoufox-reverse-mcp` 仓库、trace 内核均来自 GitHub（或 GitHub Releases）。本机若处于代理 / 透明网关 / 自签 CA 环境，直接访问 GitHub 会失败（SSL 证书校验失败、451 / 超时）。按以下顺序降级：

1. **SSL 自签 CA 兜底**：代理 MITM 用自签 CA 时，`certifi` 不信任。合并代理 CA 到独立证书包并导出：
   ```bash
   # 用 openssl s_client 抓取出口代理 CA，追加到 certifi 包
...
2. **Releases / 大文件下载用 ghproxy 镜像前缀**（实测可用）：
   ```bash
   # 原 URL
   https://github.com/daijro/camoufox/releases/download/vX.Y.Z/camoufox-...-win.x86_64.zip
   # 镜像 URL（任选其一前缀）
   https://ghproxy.net/https://github.com/daijro/camoufox/releases/download/...
   https://mirror.ghproxy.com/https://github.com/daijro/camoufox/releases/download/...
   # 或把 github.com 换成 kgithub.com
   https://kgithub.com/daijro/camoufox/releases/download/...
   ```
   `camoufox fetch` 内部走 GitHub Releases API，不可直接换源时，用上面镜像 URL 以 `curl -kL --retry 8 -C - -o camoufox.zip` 断点续传下载，再手工解压到 `python -m camoufox path` 所示缓存目录并写入 `version.json`（字段 `{"version":"X.Y.Z","release":"..."}`）。
3. **git clone 用镜像**：
   ```bash
   git config --global url."https://ghproxy.net/https://github.com/".insteadOf "https://github.com/"
   # 或
   git clone https://kgithub.com/WhiteNightShadow/camoufox-reverse-mcp.git <install-dir>
   ```
4. **raw 文件用镜像**：`https://raw.githubusercontent.com/...` → `https://raw.gitmirror.com/...` 或 `https://cdn.jsdelivr.net/gh/...`。
5. **playwright 版本匹配**：camoufox 浏览器与 playwright 协议有严格版本耦合（如 camoufox 135 需 playwright≈1.44）。`pip install camoufox` 后若 CDP `setDefaultViewport` 报 `isMobile` 等协议不匹配，固定 playwright 版本重试，不要无脑升到最新。
