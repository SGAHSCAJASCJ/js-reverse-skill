...
function renderDynamicReport(stage, data) {
  ...
  lines.push(...tableLines(data.webApis || data.webAPIs || data.apis, [
    ...
    { title: '是否 NativeProtect 保护', keys: ['nativeProtect', 'native_protect', 'protection'] },
  ...
  if (typeof protection === 'object' && !Array.isArray(protection)) {
    ...
    lines.push(`- NativeProtect 使用情况：${safeString(firstValue(protection, ['nativeProtect', 'nativeProtectUsage']))}`);
