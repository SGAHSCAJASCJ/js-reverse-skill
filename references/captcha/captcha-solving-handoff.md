...
### 自训模型（ddddocr / OpenCV 零样本失效时）

触发题型：图标点选（多目标分类）、旋转角度回归、孪生/相似度匹配——这类题 ddddocr 零样本能力不足，打码平台又不便时走自训。

最小流程：

1. **素材采集**：同一题型批量拉取（务必混入目标站真实样本，模型只认训练分布）；
2. **标注**：labelme 手工标注或自动预标注 + 人工校正（标注集划分训练/验证）；
3. **训练**：检测用 YOLO 系（定位多目标框），分类/匹配用 ResNet 多头分类或孪生网络（同类题型模型可跨站复用，**先查已有模型再训练**）；
4. **接入 solver**：输出坐标/类别 → 复用坐标换算与轨迹脚本（见下节）。

AI 辅助边界：可用大模型做素材筛选、预标注、训练脚本生成，但识别结果仍须过验证接口实测——**离线准确率 ≠ 通过率**（轨迹与环境影响通过率，见 §硬约束）；AI 不替代实测验收（经验规则 40）。
