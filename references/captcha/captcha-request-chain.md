...
三段式描述逻辑链路，**物理通道不限于 HTTP**：

| 变体 | 特征 | 排查方式 |
|---|---|---|
| HTTP(S)（主流） | load/solve/verify 均为 XHR/fetch | 常规抓包 |
| **wss 帧提交** | HTTP 只见 load，找不到 verify 请求；答案与轨迹经 WebSocket 帧上行（帧内自带编码/签名） | 查 WS 帧而非只看网络请求列表，见 `network/websocket-signing.md` |
| wasm 内编码 | verify 请求存在但载荷编码逻辑在 wasm 内 | 见 `env/env-wasm.md`，按黑盒 I/O 对齐 |

纪律：链路定位表出现「找不到 verify 请求」时**不得直接判定无验证提交**，先按上表排查通道与编码形态——把 wss 帧或 wasm 编码漏掉会得出"验证码不校验答案"的错误结论。
