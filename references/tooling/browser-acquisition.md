...
- **采集范围**：采集**本次 case 取证链路命中的资源**——目标站**及其依赖的第三方 SDK / CDN / 风控上报端点 / 验证码端点**（外站 SDK 常是加密与指纹的主体，必须一并采集），全部落盘 `case/`。**不得借取证通道采集与本次 case 无关的数据**（无关站点的 Cookie / Storage / 登录态、书签、密码、历史、扩展数据）。判据是「是否属于本次取证链路」，**不是「是否同域」**。
