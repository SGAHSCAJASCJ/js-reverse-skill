...
- 默认用 camoufox MCP 黄金路径定位签名函数；camoufox 不可用且用户已提供 JS 文件时走降级模式（纯 Grep）
...
## 无 camoufox 纯 Node 取证预案（零浏览器路径，已实测可行）

> **何时用**：本机未安装 camoufox / ruyiPage / RuyiTrace，或用户只给 URL 没给请求包。L1 标准算法签名（md5/aes/hmac/标准签名）**完全可不用浏览器解决**——本预案已端到端实测验证：`curl` 取 JS → Grep 定位 → 纯 Node 复现 → 真请求验证，全程零浏览器、零 MCP。
