...
>
> **判定顺序**：先判 env-free（参数是否同输入同输出、不依赖时间/随机/环境）——是则用 vm 输出当 oracle 反测标准哈希，升级为路径 A，砍掉整层补环境；否则再按题型表选路径。
...
| 8. 取证被目标站内核级检测阻断 | 目标请求在取证浏览器（Firefox 内核）持续 4xx 且已定位到 UA 之外的内核级检测（`eval.toString()` 分支、Error.stack 格式、引擎特征等，改 UA / `--ua` 无效） | 进入状态机 `BLOCKED_FORENSIC`（SKILL.md 第 4 节）：输出卡点与检测证据对齐用户后三选一——① 用户确认后用浏览器 MCP 连接真实 Chrome 内核浏览器兜底取证（先过 `node scripts/state_machine.js --case-dir <project-root> --guard mcp`；非 Firefox 内核与定制 Firefox 互补，match14 实证引擎检测不拦真实浏览器；成功样本 Cookie/指纹/JS/网络记录落盘 `case/`，按用户材料归类走 MATERIALS_FALLBACK 校验）；② 用户提供真实浏览器 cURL/HAR 走 MATERIALS_FALLBACK；③ 用户确认降级（义务同阻塞点 #5 例外：经验沉淀与最终总结写明 Step 2 缺失原因，REAL_VERIFY 不可豁免）。定位方法见 `references/env/env-detect-bypass.md` 内核级差异检测；**不得**静默跳过 Step 2 直接补环境，也**不得**无限重试取证 |
