...
> 适用范围：L2（MCP 调查）与 L3（Trace 模式）涉及补环境时必读；L1 纯算重写不涉及补环境，可跳过。
...
7. **普通 WebAPI 函数必须做 native-like 保护**
   不要在 WebAPI 主路径中直接写 `ctx.Blob = function(){}`、`ctx.indexedDB = { open() {} }`、`prototype = { getContext() {} }` 或 `ctx.URL.createObjectURL = function(){}`。普通函数也要提取为具名函数，并通过 `NativeProtect` helper 包装后再安装 descriptor，并在注释与阶段报告中说明保护方式。

8. **构造函数错误必须可追溯**
   构造函数的报错类型和 message 必须来自目标浏览器采样。不要写泛化 `throw new TypeError('Illegal constructor')`；需要在注释、fixture 或阶段报告中说明该错误来自哪个浏览器、哪个调用方式。

9. **native-like 实例不再二次标记对象类型**
   `markObjectType` 不是批准 API。已通过 `NativeProtect` / `createProtoChains` 处理的实例本身应已处理 `Object.prototype.toString`；普通对象才允许 `markObjectToString` / `Symbol.toStringTag`，并必须写明原因。

10. **注释说明"为什么"和"来源"**
    注释不是逐行翻译代码，而是说明模块职责、浏览器样本来源、Trace 证据、fixture 匹配规则、NativeProtect 保护决策和 fallback 原因。

11. **runtime 也必须文件化模块交付**
    补环境源码不得写成大段 `String.raw`、`CORE_SCRIPT`、`BROWSER_OBJECTS_SCRIPT`、`*_SCRIPT` 聚合字符串。正确做法是把 `navigator.js`、`document.js`、`window.js`、`canvas.js`、`webgl.js` 等作为真实源码文件保存，由宿主侧 runtime 读取后注入 Context。只有少量 bootstrap 片段允许字符串形式，且必须少于 40 行、不能承载主要 WebAPI 实现，并写明原因。
...
- 关键 getter / setter：说明真实值来自 fixture、Trace、请求样本还是用户配置。
- native-like 函数和构造函数：说明挂载位置、浏览器行为和 NativeProtect 保护处理。
- fallback 分支：说明 API 调用失败、无法覆盖或用户明确豁免的原因。
...
每个文件都应是正常 JavaScript 文件，文件顶部用中文说明模块职责。Context 内不需要也不应该直接 `require('./navigator.js')`；由宿主 runtime 读取文件内容后注入，保留源码文件结构、中文注释和 filename 栈定位。
...
| 生成大段 `String.raw` | 改为真实 `.js` 文件，由 runtime 读取后注入 Context |
...
| 普通 WebAPI 函数 | 提取具名 callback，用 `NativeProtect` helper 包装做 native-like 保护 |
