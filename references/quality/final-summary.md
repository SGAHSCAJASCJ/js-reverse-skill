...
- 必须保留"阶段报告索引""NativeProtect 使用情况""指纹基线一致性""环境与指纹 API 调用回放明细""高强度环境检测覆盖矩阵"章节。
...
- 最终项目 runtime 文件：
- Node 版本 / ABI / 平台：
- 未使用框架时是否确认无 runtime 依赖文件：是 / 否 / 不适用

## 11. 补环境实现概览

- 运行隔离方式：不使用框架 / vm / jsEnv / 独立 Node 进程 / 显式隔离 global
- Level 1 基础环境：
- Level 2 指纹真实性：
- Level 3 目标 SDK 专用环境：
- Node 泄露阻断：
...
## 12. NativeProtect 使用情况

- NativeProtect 启用状态：已启用 / 用户明确豁免（不得写"未使用"而不说明原因）
- 实际使用的 NativeProtect API：`markNativeFunction` / `markNativeGetter` / `markNativeSetter` / `createNativeCollection` / `Symbol.toStringTag` 设置 / 其他
- `document.all` 处理：HTMLDDA 近似处理 / 未涉及
- `navigator.plugins` / `navigator.mimeTypes` 处理：native-like 集合实现 / 未涉及
- 集合对象处理：native-like 原型链与 `Symbol.toStringTag` / 未涉及
- toString 保护覆盖：函数 / 构造函数 / getter / setter / 实例对象
- 阶段输出或 notes 中的补环境初始化保护记录、用户豁免记录：

## 12.1 native 能力缺口闭环

- 是否发生 native 能力缺口：未发生 / 已发生
- 阻塞 API / 行为：
- 触发位置与证据：Trace / Hook / Node trace / 目标源码位置 / fixture
- 真实浏览器基线摘要：
...
- 用户选择与原因：临时 workaround / 暂停 case / 其他
...
- 数据来源：Trace NDJSON / Node trace / Proxy trace / Hook / fixture / 静态分析 / 手动确认
...
按实际涉及的分类补充表格，每条精确到 API 路径、类型、所属对象 / 原型、来源证据、调用参数摘要、返回值 / 回放值摘要、补环境实现、真实性保护、验证结果和备注。
...
| toString 多通道 | 未涉及 / 已涉及 | 已采样 / 未采样 | 已对比 / 未对比 | 通过 / 部分通过 / 未修复 | NativeProtect 需覆盖多通道 |
...
| Permissions / Plugins / MimeTypes | 未涉及 / 已涉及 | 已采样 / 未采样 | 已对比 / 未对比 | 通过 / 部分通过 / 未修复 | native-like 集合，禁止普通数组 / 空对象主路径 |
...
- 模块拆分情况：
- 单文件 / 单函数复杂度：
- 中文注释覆盖：文件头 / WebAPI / getter / setter / NativeProtect 保护 / fallback / 指纹回放 / 加密入口
