...
> **触发条件**：进入 Node.js 补环境阶段、目标 JS 创建或访问 `iframe`、`frame`、`contentWindow`、`contentDocument`、`postMessage` 跨 frame 通信、验证码 SDK iframe 等场景时读。

本文件聚焦 iframe 的 realm 隔离、跨 frame 通信、取证方式与补环境模拟策略；通用对象模型遵循 `env-object-model.md`，行为矩阵门禁遵循 `webapi-env-detection-matrix.md`。
