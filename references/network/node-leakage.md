...
- `Navigator` / `navigator` 是 Node v21.0.0 新增；`navigator.userAgent` 自 v21.1.0 起可能返回 `Node.js/<major>`。
...
- 浏览器环境对象模型补环境：见 `env/env-object-model.md`
- 真实性保护与 native-like：见 `env/env-native-protection.md`
- 补环境调试循环：见 `env/env-debug-loop.md`
- Cookie 静默失败与请求链：见 `network/session-chain.md`
