...
| 机制 | 典型证据 | 破解方向 |
|---|---|---|
| **蜜月期窗口** | 页面加载（GET HTML）后短时间内 API 放行、成功请求续期；窗口外同构造请求被拒；重放"5 秒内成功、15 秒失败" | 纯协议先 GET HTML 开窗（curl_cffi 即可），并保持请求间隔小于续期窗口 |
| **链式参数形态（反直觉）** | 签名参数在客户端跨请求累加（如 `q = 1-<t1>\|1-<t2>\|`），容易诱导"服务器校验链衔接"的假设 | **实测 match6 服务端不校验链**：单段链全通过、纯协议多段链反被拒——浏览器端参数行为 ≠ 服务器要求；别照抄浏览器参数形态，用最小构造二分验证 |
| **"频率墙"误判（高频陷阱）** | 风控文案含"放慢速度/别匆忙"等字样，诱导按字面归因频率限制 | **match6 实测无频率墙**：签名正确时无缝连发（65ms/页）全过；"加 sleep 后能过"的真实机制是失败累积触发惩罚态、sleep 给了冷却时间——先确认签名正确，再对照浏览器语义（翻页即点即发、无人工延迟）证伪频率墙假设 |
| **失败惩罚计数** | 连续失败 N 次（约 5-10）后连浏览器自身请求也被拒；冷却 10 分钟+恢复 | 诊断前先验基线（页面加载看数据渲染）；失败预算 3-5 次即停冷却；惩罚期内不做任何实验 |
| **数据绑定会话（归因陷阱）** | 数据随请求变化，诱导归因"平台数据按周期重置"或"IP 限流" | **会话绑定验证**：固定会话凭证（cookie/Authorization 等）重放，数据恒定 = 数据绑定会话（match9 实测：匿名会话每次数据不同、固定会话数据恒定）；数据接口先确认会话绑定再归因平台行为 |
| **随机边缘拒绝（时好时坏）** | 同一构造提交结果随机通过/拒绝（10~50% 成功），失败与成功返回相同挑战文案；易被误判为"时间窗口"并盲试 sleep/延迟参数 | **先 ≥10 次大样本统计**确认"随机拒绝 vs 固定条件窗口"（match9 实测：盲试 ts 年龄 1.0~2.5s 与精确控龄多轮全是随机边缘拒绝，vm m 50% vs 浏览器 eval m 100%）；签名语义经对照实验确认后**停止调参**，用重试机制兜底（≤8 次、间隔 2s，50% 成功率下 ≈99.6% 通过）；单次失败不构成签名错误证据（详见经验规则 23） |

识别纪律：
- 判定成败只看**响应体**（业务数据 vs 风控文案），客户端状态更新（window 变量累加、DOM 变化）不代表请求成功
- 风控文案**不区分具体原因**（签名错/超窗/惩罚全同文案，如"别匆忙，放慢速度"），且字面语义可能完全误导（match6 实测"放慢速度"实为签名错误信号、无频率墙），需按层逐项排查，不能按文案字面归因
- 这类站点常同时要求完整 cookie 链（含统计类 cookie 如 `Hm_lvt`）与特定页 UA 门槛（如末页 `UA: yuanrenxue`），对照时把 cookie 完整性当作独立变量测
- 数据接口出现"数据变化、与预期不符"时，**先固定会话凭证重放**验证数据是否绑定会话，再归因"平台重置/IP 限流/风控"（match9 实测"数据周期重置"是假象，真实机制是数据绑定会话；会话凭证也可能由 API 响应 set-cookie 下发而非页面）

## 行为/模型层天花板（客户端逆向的边界）

> **触发条件**：签名内容层双对照全 200、会话基线稳定、失败率大样本统计稳定，但目标业务动作（提交/写/连续读）仍被拒——拦截发生在服务端行为模型层，此时读本节止损。

判据（三条同时满足才下「行为层」结论）：① 正反对照均 200（内容层 + 连接层已排除）；② 每次实验前复刻基线成功（会话/惩罚态已排除）；③ 换 IP、换签名实现、换浏览器内核后失败率无显著变化（排除实现与出口变量）。

排查项（先补链路完整性，再谈绕过）：

| 排查项 | 要点 |
|---|---|
| 链路完整性 | 设备类站点把「设备注册」当作设备生命周期事件，往往不是一次请求：注册 → 环境/日志上报 → 业务请求构成完整序列。只复刻业务请求而漏掉注册/上报环节，服务端设备轨迹不完整同样拒。先对照浏览器侧完整链路（含隐形上报请求）逐条补齐 |
| 字段同源性 | 注册与上报、上报与业务请求之间共享的设备标识/指纹字段必须同源一致（同一设备值贯穿全链路），拼凑来源不同批次的值会被判为伪造 |
| 行为序列 | 请求节奏、翻页模式、操作间隔是否符合真人分布（参考 `fingerprint/` 与验证码轨迹章节）；高频无延迟连发在「无频率墙」站点通过、在有行为模型的站点会被单独统计 |

纪律：本层无法从客户端代码推导（模型与阈值在服务端），**据此写结论并止损**——不得以换代理、换内核、加 sleep、盲调参数的方式继续碰运气。报告明确写「拦截在服务端行为/数据模型层，客户端侧已对齐，不在本技能可绕过范围」，并把已排除项（内容层/连接层/会话层）与证据一并留档。
