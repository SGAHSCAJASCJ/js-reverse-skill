...
| 6 | 服务端静默拒绝 | 环境不完整时返回 HTTP 200 + 空 body，不返回错误码 | 通过对比 trace 取证 真实环境逐项排查 58 项差异 |
| 7 | ttwid Cookie 无法通过协议获取 | `POST /ttwid/check/` 返回 `status_code: 1002, "check not pass"`；首页 `GET /` 的 `set-cookie` 只返回 `__ac_nonce`，不返回 ttwid | ttwid 是浏览器端 JS 生成后写入的，纯协议无法直接获取。**解决方案**：从 trace 取证 调试浏览器的 Cookie 中导出完整 cookie_str 硬编码到脚本中（含 ttwid、odin_tt 等 httpOnly Cookie）。ttwid 有效期约 1 年，不需要频繁更新。⚠️ 注意：此做法与红线 4 存在冲突。正式交付时必须将 ttwid 等关键业务 Cookie 作为外部配置输入（环境变量/配置文件），不得硬编码到源码中；或通过 Session 请求链在运行时获取。 |
...
| 19 | 对 Object.prototype / Array.prototype 做 markNative | 破坏 JSVMP 自身运行，签名异常 | 不要对 Object.prototype / Array.prototype 做 markNative |
| 20 | SDK 脚本每次从 CDN 下载 | 加载不稳定，且无法离线调试 | SDK 脚本必须保存到本地（`scripts(action='save')`），不要每次从 CDN 下载 |
| 21 | 环境补丁批量添加后排查 | 无法定位是哪一项导致签名被拒 | 环境补丁逐项添加逐项验证，不要批量添加后排查 |
| 22 | Firefox 和 Chrome 环境混补（UA 与 API 不自洽）| 签名 192 字符但服务端返回空 body | Firefox 和 Chrome 环境都可以工作，关键是所有值与 UA 自洽 |
...
用 trace 取证 反检测浏览器（C++ 引擎级指纹伪装）作为「干净基准」，在目标页面上下文中执行环境采集代码，与 jsdom 输出逐项 diff。分批采集（navigator / screen+window / document+performance+toString / DOM+Canvas+WebGL+Audio）避免单次执行过长。
...
1. a_bogus 参数长度 180-192 字符
2. navigator.webdriver === false
3. 签名输入包含完整 query string,不含 #hash
4. webmssdk.es5.js / bdms.js / sdk-glue.js 三件套必须同时加载
5. bdms.paths 配置必须在 SDK init 前完成
6. XHR 拦截器追加 a_bogus 到 URL,fetch 拦截器不追加
7. a_bogus 由 JSVMP 生成,frontierSign 生成 X-Bogus(两者不同)
8. jsdom 环境需补 58 项(见"环境检测差异全表")
9. Firefox UA 下 Function.prototype.toString 返回 native code 格式
10. Cookie 中 ttwid/__ac_nonce/__ac_signature 为动态字段
...
| 参考文档 | 关联点 |
|---------|--------|
| `references/env/env-object-model.md` | 补环境对象模型硬性清单（58 项差异对齐依据） |
| `references/env/env-native-protection.md` | Function.prototype.toString 三层防御 + markNative 实现 |
| `references/env/env-detect-bypass.md` | 环境检测绕过清单（navigator.webdriver / plugins / DOM 布局） |
| `references/hooks/hook-templates.md` | XHR Hook 模板 + "只观察不篡改"纪律 |
| `references/network/session-chain.md` | Session 请求链（ttwid 等动态 Cookie 获取） |
| `references/network/tls-validation.md` | TLS 指纹对齐（axios vs got-scraping） |
| `references/fingerprint/fingerprint-value-replay.md` | 指纹基线 3 层值来源优先级 |
| `references/workflow/decision-tree.md` | JSVMP 路径 A vs 路径 D 决策 |
| `references/workflow/experience-rules.md` | 规则 1/3/5/12/16/18（Hook 顺序 / 入口确认 / toString / 环境补丁时序） |
