...
关键发现：
  - trace 取证 是 Firefox 内核，Function.prototype.toString 格式与 Chrome 不同
...
| # | 坑 | 现象 | 解决方法 |
|---|---|------|---------|
| 1 | Firefox vs Chrome 的 native code 格式差异 | trace 取证 基于 Firefox，toString 含换行缩进；jsdom 默认输出 Chrome 格式单行 | markNative 输出 Firefox 格式：`function name() {\n    [native code]\n}` |
...
| Chrome 环境伪装 | 如果不用 trace 取证 而用 Chrome 采集基准 | markNative 改回 Chrome 单行格式 |

---

## 浏览器指纹采集方法

使用 trace 取证 在 Firefox 内核浏览器中采集指纹：
...
trace 取证 基于 Firefox 内核，其 `Function.prototype.toString` 对原生函数返回的格式与 Chrome 不同。JSVMP 会检测这个格式来判断浏览器类型。如果 jsdom 的 markNative 输出 Chrome 格式，会导致浏览器类型判断不一致，签名被拒。
...
Node.js 原生 HTTP 客户端的 TLS 指纹会被识别。使用 got-scraping 可以模拟 Firefox 的 TLS Client Hello 指纹，与 trace 取证 采集的环境保持一致。
...
1. X-Bogus 长度 180-192 字符
2. X-Gnarly 长度因请求而异(动态)
3. navigator.webdriver === false
4. cacheOpts 初始化必须在 SDK init 前完成
5. X-Bogus 由 XHR 拦截器追加到 URL,X-Gnarly 由 fetch 拦截器注入 header
6. 双签名变体含 cacheOpts,单签名(a_bogus)不含
7. Firefox UA 下需补 native code 格式 toString
8. jsdom 环境需补 62 项(见"环境检测差异全表")
9. webmssdk.es5.js / bdms.js / sdk-glue.js 三件套 + cacheOpts 配置
10. got-scraping TLS 指纹模拟 Firefox 115+
...
| 参考文档 | 关联点 |
|---------|--------|
| `references/env/env-object-model.md` | 补环境对象模型（62 项差异 = 58 基础 + 4 Firefox 特有） |
| `references/env/env-native-protection.md` | Firefox 格式 native code 伪装（含换行缩进） |
| `references/network/tls-validation.md` | got-scraping Firefox TLS 指纹模拟 |
| `references/fingerprint/fingerprint-baseline-consistency.md` | 指纹基线一致性（Firefox vs Chrome 格式差异） |
| `references/workflow/decision-tree.md` | JSVMP 双签名 = 双通道拦截决策 |
| `references/workflow/experience-rules.md` | 规则 1/3/12/14/16（Hook 顺序 / 入口确认 / 持久化 / compare_env / toString） |
| `cases/jsvmp-xhr-interceptor-env-emulation.md` | 国内版单签名基础案例（58 项差异表来源） |
