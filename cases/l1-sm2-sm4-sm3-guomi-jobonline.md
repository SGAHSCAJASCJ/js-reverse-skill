# L1 纯算还原：SM2/SM4/SM3 国密算法（jobonline.cn）
...
1. **CHECK-1 全量检测耗时**：`check_external_tools.js` 一次性检测所有工具（10+ 次 spawnSync），L1 纯算场景不需要。**已优化**：增加 `--quick` 模式。
2. **CHECK-3 后跳过用户确认**：AI 在 CHECK-3 完成后直接进入 Phase 0，没有问用户选择哪种分析路径。**已优化**：CHECK-3 增加用户确认门禁。
3. **camoufox 不可用时默默走静态分析**：AI 没有告知用户有"安装 camoufox 走动态调试"的选项。**已优化**：camoufox 不可用时不默默走静态分析，必须先告知用户有安装选项，由用户选择。
4. **静态分析效率问题**：2.25MB 压缩 JS 需要多次 Grep + 上下文提取才能定位算法，效率不如 camoufox MCP 的 `get_request_initiator` 直接定位签名函数。

### 静态分析 vs 动态调试

| 维度 | 纯静态分析（降级） | camoufox 动态调试（首选） |
|---|---|---|
...
| 依赖 | 仅 Node.js | 需 camoufox + MCP |

**结论**：对于参数名明确、JS 结构清晰的 L1 场景，静态分析是合理选择；对于大文件或参数名不明确的场景，camoufox 动态调试效率更高。camoufox 不可用时不默默走静态分析，应告知用户有安装选项。
