...
进入交付产物的可复用资源（skill 规范：assets = files that belong in generated output）。与 references（知识文档）、scripts/ast-patterns（可执行工具链）、assets/templates（交付骨架）、cases（经验库）的关系：

| 目录 | 性质 | 使用方式 |
|---|---|---|
| `env-patch-snippets/` | 补环境代码片段 | 可被 `assets/templates/` 直接 require |
...
> **已内联进交付模板**：`assets/templates/vm-sandbox/native-protect.js` 是同一份文件的副本。复制 `vm-sandbox/` 到 `result/src/env/` 后，`vm-context.js` 优先 `require('./native-protect.js')`，**交付物不再依赖 skill 仓库的 `assets/`**，可整体搬走到任意位置。
...
> AST 反混淆工具链已按 skill 规范归位到 `scripts/ast-patterns/`（可执行代码属 scripts，不属 assets）：14 流水线步骤（7 通用 + 7 站点专用，以 STEP_LIBRARY 为准），触发条件为 OB/CFF/eval 混淆且需 AST 反混淆后才能提取算法。
