...
function geetest4GuardedPass(ast) {
  ...
  if (body.length >= 5) {
    ...
    const decryptPropName = body[2] && body[2].expression && body[2].expression.left ? generateCode(body[2].expression.left) : "";
    const ctrlVarName = body[3] && body[3].expression && body[3].expression.left ? generateCode(body[3].expression.left) : "";

    if (decryptPropName && evaluateNodes(body.slice(0, 5), sandbox)) {
      traverse(ast, {
        VariableDeclaration(path) {
          if (
            path.node.declarations.length !== 3 ||
            !path.get("declarations.0.init").node ||
            generateCode(path.get("declarations.0.init").node) !== decryptPropName
          ) {
            return;
          }
          ...
          candidateNames.forEach((name) => {
            ...
            binding.referencePaths.forEach((refPath) => {
              ...
              const result = evaluateExpression(`${decryptPropName}(${argValue})`, sandbox);
              if (!result.ok) {
                return;
              }
