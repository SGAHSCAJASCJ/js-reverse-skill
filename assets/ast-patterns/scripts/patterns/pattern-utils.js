...
function replaceMembersFromSandbox(ast, sandbox) {
  ...
  traverse(ast, {
    MemberExpression(path) {
      ...
      const key = resolvePropertyKeyNode(path.node.property, { sandbox });
      if (key === null) {
        return;
      }
...
function collectWrapperDescriptor(node, wrapperMap, options = {}) {
  ...
  if (t.isMemberExpression(node) && t.isIdentifier(node.object)) {
    const key = resolvePropertyKeyNode(node.property, options);
    if (key !== null) {
  ...
  if (t.isFunctionExpression(node)) {
    ...
    if (t.isCallExpression(argument) && t.isMemberExpression(argument.callee) && t.isIdentifier(argument.callee.object)) {
      const key = resolvePropertyKeyNode(argument.callee.property, options);
      if (key !== null) {
...
function applyWrapperObjects(ast, wrapperMap, options = {}) {
  ...
  traverse(ast, {
    CallExpression(path) {
      ...
      const key = resolvePropertyKeyNode(path.node.callee.property, options);
    ...
    MemberExpression(path) {
      ...
      const key = resolvePropertyKeyNode(path.node.property, options);
      const resolved = resolveDescriptor(wrapperMap, { type: "member", objectName, key });
