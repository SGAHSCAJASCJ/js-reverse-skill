...
function getOpcodeTestInfo(testNode) {
  ...
  if (t.isNumericLiteral(testNode.left) && t.isIdentifier(testNode.right)) {
    return {
      discriminant: clone(testNode.right),
      discriminantName: testNode.right.name,
      caseValue: clone(testNode.left)
    };
  }

  if (t.isNumericLiteral(testNode.right) && t.isIdentifier(testNode.left)) {
    return {
      discriminant: clone(testNode.left),
      discriminantName: testNode.left.name,
      caseValue: clone(testNode.right)
    };
  }
...
function collectOpcodeCasesFromAlternate(node, discriminantName, cases) {
  ...
  if (t.isIfStatement(node)) {
    const info = getOpcodeTestInfo(node.test);
  ...
  if (t.isBlockStatement(node)) {
    node.body.forEach((stmt) => {
      if (t.isIfStatement(stmt)) {
        const info = getOpcodeTestInfo(stmt.test);
...
function convertOpcodeIfChains(ast) {
  ...
  traverse(ast, {
    IfStatement(path) {
      const info = getOpcodeTestInfo(path.node.test);
      if (!info) {
        return;
      }
