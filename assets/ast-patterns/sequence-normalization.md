...
- `ReturnStatement` 的 `SequenceExpression` 已实现拆分：前面的表达式提到 `return` 之前作为独立语句，保留最后一个表达式作为 `return` 的参数。
- `VariableDeclarator` 的 `SequenceExpression` 拆分仍需谨慎，没有足够稳定的模板时宁可不动。
